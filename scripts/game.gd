...
func receive_orders(received_actions: Array[PackedStringArray], selected_system_ids: Array[PackedInt32Array], selected_ship_ids: Array[PackedInt32Array]) -> void:
	...
	if not galaxy.in_setup and self.turn_orders.keys().size() == self.player_ids.size():
		...
		print(self.turn_orders)
		
		var turn_array: Array[Array] = self.TurnProcessor.process_turn(self.galaxy, self.turn_orders)
		...
		print("Orders received, but more orders still needed before turn can be processed.")
...
You possess $num_systems system$sys_s, $num_constructions construction$const_s and $num_ships ship$ship_s."
	...
	var desc_format: Dictionary = {
		...
		"$tech_level": str(self.galaxy.factions[self.galaxy.player_id].calculate_tech_level()),
