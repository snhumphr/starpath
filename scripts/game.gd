	...
	if event is InputEventMouseMotion:
		...
		if self.highlighted["highlight_type"] == "system":
			...
			if event.button_index == MOUSE_BUTTON_LEFT:
				...
				if not is_selected:
					selections.add_ship(galaxy, system)
			...
			if starpath_systems.size() == 2 and self.selections.selected_systems.size() > 0:
				...
				if starpath_systems[0].is_system_identical(latest_system):
					...
					if not is_selected:
						selections.add_ship(galaxy, starpath_systems[1])
					self.update_map()
					...
					if not is_selected:
						selections.add_ship(galaxy, starpath_systems[0])
					self.update_map()
...
func _on_button_pressed() -> void:
	...
	if self.galaxy.in_setup and self.action_queue.size() == 0:
		...
		for action in self.action_queue:
			if not action.is_action_executable(self.galaxy.player_id, self.galaxy, action.bound_action_selection):
				end_turn_allowed = false
				break
